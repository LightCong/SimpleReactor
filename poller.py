...
class KqueuePoller(object):
	...
	def poll(self,timeout):
		...
		import select,error
		try:
			re_kevent_lst=self.kq.control(None,self.channel_count,timeout)
		except select.error, err:
			if err.args[0] != error.EINTR:
				# poller error
				raise Exception()
	...
	def update_channel(self,channel):
		...
		if channel.need_read==True:
			# 增加读监听,重复增加不会有影响
			kevent=select.kevent(channel._fd,filter=select.KQ_FILTER_READ,flags=select.KQ_EV_ADD|select.KQ_EV_ENABLE)
		...
		if channel.need_write==True:
			#增加写监听,重复增加不会有影响
			kevent = select.kevent(channel._fd, filter=select.KQ_FILTER_WRITE,flags=select.KQ_EV_ADD | select.KQ_EV_ENABLE)
			change_event_lst.append(kevent)
			pass
